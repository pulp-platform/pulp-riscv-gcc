/*
 * Offload image generation tool for RISC-V PULP MRWOLFE device.
 *
...
#include "mrwolfe-offload.h"

const char tool_name[] = "MRWOLFE mkoffload";
...
prepare_target_image (const char *target_compiler, int argc, char **argv)
{
  ...
  /* Add MRWOLFE compilation flags  */
  const char *PULP_MRWOLFE_EXTCFLAGS = getenv("PULP_MRWOLFE_EXTCFLAGS");
  char **mrwolfe_cflags = buildargv(PULP_MRWOLFE_EXTCFLAGS);
  const char *PULP_MRWOLFE_EXTLDFLAGS = getenv("PULP_MRWOLFE_EXTLDFLAGS");
  char **mrwolfe_ldflags = buildargv(PULP_MRWOLFE_EXTLDFLAGS);
  ...
  for(int i = 0; i < countargv(mrwolfe_cflags); i++)
    obstack_ptr_grow (&argv_obstack, mrwolfe_cflags[i]);
  ...
  for(int i = 0; i < countargv(mrwolfe_ldflags); i++)
    obstack_ptr_grow (&argv_obstack, mrwolfe_ldflags[i]);
  obstack_ptr_grow (&argv_obstack, "-o");
  ...
  freeargv(mrwolfe_cflags);
  freeargv(mrwolfe_ldflags);
  ...
  obstack_ptr_grow (&argv_obstack, "elf32-littleriscv");
...
{
  progname = "mrwolfe-mkoffload";
  gcc_init_libintl ();

  // /* Diagnostic -- Start */
  diagnostic_initialize (global_dc, 0);
  inform(input_location, "PULP MRWOLFE mkoffload start...\n");
  // /* Diagnostic -- End */
  ...
  struct obstack argv_obstack;
  obstack_init (&argv_obstack);
  obstack_ptr_grow (&argv_obstack, "riscv32-unknown-elf-ld");
  obstack_ptr_grow (&argv_obstack, "-melf32-littleriscv");
  ...
  /* Diagnostic -- Start */
  inform(input_location, "PULP MRWOLFE mkoffload end.\n");
  /* Diagnostic -- End */
  return 0;
}
